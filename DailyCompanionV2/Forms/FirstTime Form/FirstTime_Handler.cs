...
namespace DailyCompanionV2.Forms.FirstTime_Form
{
            ...
            , Label MsgLabel, ProgressBar progBar, Control sender)
        {
            ...
            try
            {
                ...
                using (IDbConnection connect = Database_Connection.Create_Connection<SQLiteConnection>())
                {
                    ...
                    await HM_Manager.Email().SendMailAsync(HM_Manager.EMessage(user_email, FirstTimeEmail_Helper.GetBody(user_name, serial, user_address, user_phonenumber, user_email, mainPassword, coWebpage),"DailyCompanionV2 Corp,Inc for personal use apps, خدمة عملاء"));
                    Console.WriteLine("Done");
