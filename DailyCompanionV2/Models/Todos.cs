...
namespace DailyCompanionV2.Models
{
    public class Todos
    {
        ...
        private string chkString
        {
            get
            {
                if (chkpoint_list != null)
                    return string.Join(" ", chkpoint_list.Where(p => p != null).Select(p => p.ChkName));
                else return "";
            }
        }
        ...
        public string String => $"{id} {duefrom} {dueto} {category} {todo} {startdate} {duedate} {str_done_date} {duration} {notes} {chkString}";
    }
}
