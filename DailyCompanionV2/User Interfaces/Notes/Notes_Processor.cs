...
namespace DailyCompanionV2.User_Interfaces.Notes
{
    public static class Notes_Processor
    {
        ...
        public static bool Handle_Note(Enums.genericHandle_Type handleType, GroupBox grpbox, string title, string note,Label MsgLabel, string Title, Color color, Models.Notes oldModel, bool byPass = false)
        {
            ...
            int _id = id;
            Models.Notes _noteNew = new Models.Notes() {id = _id, title = title, strNote = note,lst_updt = DateTime.Now };
            ...
            switch (handleType)
            {
                case Enums.genericHandle_Type.إضافة:
                    tobeDone = delegate ()
                    {
                        DBHelper.Insert_Database("notes", new List<string> { "title", "note", "lstupdt" }, new List<Models.Notes> { _noteNew }, HM_Manager.HandleHistory, new string[] { "الملاحظات", "إضافة ملاحظة", "" });
                ...
                case Enums.genericHandle_Type.تعديل:
                    tobeDone = delegate ()
                    {
                        DBHelper.Update_Database("notes", new List<string> { "title", "note", "lstupdt" }, new List<Models.Notes> { _noteNew }, HM_Manager.HandleHistory, new string[] { "الملاحظات", "تعديل ملاحظات", $"العنوان الخاص بالمهمة: {_noteNew.title}" });
