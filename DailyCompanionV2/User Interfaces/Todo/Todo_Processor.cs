...
namespace DailyCompanionV2.User_Interfaces
{
    public static class Todo_Processor
    {
        public static bool HandleTODO(Enums.genericHandle_Type handleType, GroupBox grpbox, string duefrom, string dueto, string todo, string category, DateTime startDate, DateTime dueDate,bool done_dateDone, DateTime donDate,DateTime addedDate,double duration, string notes,List<Checkpoint> chkList,Label MsgLabel, string Title, Color color, Todos oldModel ,bool byPass = false)
        {
            ...
            Todos _todoNew = new Todos() { id = id, duefrom = duefrom, dueto = dueto, todo = todo, category = category,start_date = startDate, due_date = dueDate, done_date = doneDate, duration = duration, notes = notes, added_date = addedDate,chkpoint_list = chkList, lst_updt = DateTime.Now };
            int _id = id;
            ...
            switch (handleType)
            {
                case Enums.genericHandle_Type.إضافة:
                    tobeDone = delegate ()
                    {
                        ...
                        _id = DBHelper.Insert_Database("todos", new List<string> { "category", "duefrom", "todo", "dueto", "duedate", "notes", "addeddate", "donedate", "duration","startdate","chklist","lstupdt" }, new List<Todos> { _todoNew }, HM_Manager.HandleHistory, new string[] { "المهام المطلوبة", "إضافة مهمة", "" });
                ...
                case Enums.genericHandle_Type.تعديل:
                    tobeDone = delegate ()
                    {
                        DBHelper.Update_Database("todos", new List<string> { "category", "duefrom", "todo", "dueto", "duedate", "donedate", "notes", "addeddate", "duration", "startdate", "chklist", "lstupdt" }, new List<Todos> { _todoNew }, HM_Manager.HandleHistory, new string[] { "المهام المطلوبة", "تعديل البيانات", $"الكود الخاص بالمهمة: {_id}  المتعلقة ب: {dueto}" });
