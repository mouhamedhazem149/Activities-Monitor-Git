...
namespace OpsMoi.User_Interfaces.Finance
{
    ...
    {
        public static bool HandleFNC(Enums.genericHandle_Type handleType, GroupBox grpbox, string relatedentity, Enums.financeType type, int wallet, string category, double due, double paid, DateTime due_date, bool done_dateDone, DateTime doneDate, string notes, Label MsgLabel, string Title, Color color, Finances oldModel, bool byPass = false)
        {
            ...
            switch (handleType)
            {
                case Enums.genericHandle_Type.إضافة:
                    tobeDone = delegate ()
                    {
                        _id = DBHelper.Insert_Database("finances", new List<string> { "relatedentity", "type", "wallet", "category", "due", "paid", "duedate", "notes", "donedate" }, new List<Finances> { _fncNew }, HM_Manager.HandleHistory, new string[] { "العمليات المالية", "إضافة مهمة", "" });
                ...
                case Enums.genericHandle_Type.تعديل:
                    tobeDone = delegate ()
                    {
                        DBHelper.Update_Database("finances", new List<string> { "relatedentity", "type", "wallet", "category", "due", "paid", "duedate", "notes", "donedate" }, new List<Finances> { _fncNew }, HM_Manager.HandleHistory, new string[] { "العمليات المالية", "تعديل البيانات", $"الكود الخاص بالعملية: {_id}  المتعلقة ب: {relatedentity}" });
